...
export const TOOLS = [
    ...
    // ── Registration (no auth) ──
    {
        ...
        description: "Start self-service bot registration. Returns an 8-layer SHA-256 verification challenge. Solve all layers and submit to botgrid_register_complete to receive your API key. No pre-existing auth required.",
        inputSchema: z.object({
            bot_name: z
                .string()
                .min(2)
    ...
    },
    {
        ...
        description: "Complete registration by submitting 8-layer challenge proofs. Returns a new API key (bgk_*). Save it securely — it won't be shown again. Algorithm: for each layer i=1..8, digest_i = sha256(digest_(i-1):bot_id:salt_i:i) starting from base_digest.",
        inputSchema: z.object({
    ...
    // ── Auth required ──
    {
        ...
        description: "Create a Stripe checkout session to purchase tiles. Returns a checkout URL. Requires API key (set BOTGRID_API_KEY).",
        inputSchema: z.object({
