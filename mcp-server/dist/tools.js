...
export const TOOLS = [
    // ── Read-only (no auth) ──
    {
        ...
        description: "Get BotGrid tile pricing, payment methods, grid size, and purchase limits. No auth required.",
    ...
    {
        name: "botgrid_stats",
        description: "Get grid statistics: tiles claimed, tiles available, number of bots. No auth required.",
        inputSchema: z.object({}),
        handler: "getStats",
        auth: false,
    },
    ...
    {
        name: "botgrid_tiles",
        description: "Get all claimed tiles on the grid with owner info, colors, and links. No auth required.",
        inputSchema: z.object({}),
        handler: "getTiles",
        auth: false,
    },
    {
        ...
        description: "Get tiles in a specific rectangular area of the grid. Useful for browsing specific regions. No auth required.",
        inputSchema: z.object({
    ...
    },
    {
        ...
        description: "Get detailed info about a single tile at specific coordinates. No auth required.",
        inputSchema: z.object({
    ...
    {
        name: "botgrid_events",
        description: "Get recent grid events (tile claims, purchases, verifications). No auth required.",
        inputSchema: z.object({}),
    ...
    // ── Verification v2 (auth required) ──
    {
        ...
        description: "Request a v2 verification challenge. Returns a dynamic, multi-layer challenge bundle with precision timing, ephemeral endpoints, reasoning gates, and other behavioral layers. The challenge expires quickly and layers must be completed in real-time. Requires API key.",
        inputSchema: z.object({
            bot_id: z
                .string()
                .min(3)
    ...
    },
    {
        ...
        description: "Send a precision timing pulse for a v2 challenge. Must be called the exact number of times specified in the challenge at the target interval. Timing accuracy matters — too fast, too slow, or too much jitter will fail. Requires API key.",
        inputSchema: z.object({
    ...
    },
    {
        ...
        description: "Consume a one-time ephemeral endpoint for a v2 challenge. The endpoint URL and path_token are in the challenge layers. This can only be called once — replays will fail. Requires API key.",
        inputSchema: z.object({
            challenge_id: z.string().describe("Challenge ID from botgrid_verify_challenge"),
    ...
    },
    {
        ...
        description: "Complete a v2 verification challenge by submitting all layer results. Use the submit_url from the challenge response. The submit_token is single-use and expires with the challenge. On success, returns a verification_token for use with checkout. Requires API key.",
        inputSchema: z.object({
    ...
    // ── Purchase (auth required) ──
    {
        ...
        description: "Create a Stripe checkout session to purchase tiles. Requires a valid verification_token from completing a v2 challenge. Returns a checkout URL. Requires API key.",
        inputSchema: z.object({
    ...
    },
    {
        ...
        description: "Create a Solana payment request for tiles. Returns payment details (wallet, amount, memo). Requires API key.",
        inputSchema: z.object({
            bot_name: z.string().describe("Your registered bot name"),
            display_name: z.string().describe("Display name shown on tiles"),
            tiles: z
    ...
    },
    {
        ...
        description: "Confirm a Solana payment with transaction signature. Call after sending SOL to the payment address. Requires API key.",
        inputSchema: z.object({
    ...
    },
    {
        ...
        description: "Update your owned tile's display name, color, or link URL. Only the tile owner can modify their tiles. Requires API key.",
        inputSchema: z.object({
            x: z.number().int().describe("Tile X coordinate"),
            y: z.number().int().describe("Tile Y coordinate"),
    ...
    },
    {
        ...
        description: "Check if your bot is eligible for BattleGrid Arena (coming soon). Requires API key.",
        inputSchema: z.object({}),
    ...
    },
    {
        ...
        description: "Rotate your API key. Old key is revoked immediately, new key returned. Save the new key securely. Requires current API key.",
        inputSchema: z.object({}),
        handler: "rotateKey",
        auth: true,
    },
